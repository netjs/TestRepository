...
public class PreparedStatDemo {
	...
	public static void main(String[] args) {
		insertStaffData("Shanmuga", 2, 1, LocalDate.of(2017, 6, 16), LocalDate.of(1985, 8, 18),
				60000, 9);
		//updateStaffData(20, 62000, 9);
	}
	
	private static void insertStaffData(String staffName, int designationCd, 
			int departmentCd, LocalDate hireDate, LocalDate staffDOB, 
			double salary, int mgrCd) {
	...
	private static void updateStaffData(int staffCd, double salary, int mgrCd) {
		...
		try	{
			...
			String sql = "UPDATE staff_master set salary=?, mgr_code=? WHERE staff_code = ?";
			PreparedStatement pstmt = conn.prepareStatement(sql); 
	...
	private static void deleteStaffData(int staffCd) {
		...
		try	{
			...
			String sql = "DELETE FROM staff_master WHERE staff_code = ?";
			PreparedStatement pstmt = conn.prepareStatement(sql); 
